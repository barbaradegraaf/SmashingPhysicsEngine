...
namespace smashing {
	...
	inline real Vector3::magnitude() const{
		return real_sqrt(x*x + y*y + z*z);
	}
	inline real Vector3::squaredMagnitude() const{
		return (x*x + y*y + z*z);
	}
	...
	inline void Vector3::addScaledVector(const Vector3 &vector, real scale){
		x += vector.x * scale;
		y += vector.y * scale;
		z += vector.z * scale;
	}
	...
	inline Vector3 Vector3::componentProduct(const Vector3 &vector) const{
		return Vector3(x * vector.x, y * vector.y, z * vector.z);
	}
	// This is do the same 'component' product but will do it to the current this vector
	inline void Vector3::componentProductSelf(const Vector3 &vector){
		x *= vector.x;
		y *= vector.y;
		z *= vector.z;
	}
	// This does the dot product with the this value and returns the scalar value
	inline real Vector3::dotProduct(const Vector3 &vector)const{
	...
	inline void Vector3::operator*=(const real value){
		x *= value;
		y *= value;
		z *= value;
	}
	...
	inline real Vector3::operator*(const Vector3 &vector) const{
		return x*vector.x + y*vector.y + z*vector.z;
	}
